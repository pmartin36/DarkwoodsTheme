...
public class PoissonDiscSampler {
	...
	public PoissonDiscSampler(float width, float height, float radius, IEnumerable<GridPos> illegalGrids = null) {
		...
		if(illegalGrids != null) {
			foreach(var g in illegalGrids) {
				grid[g.x, g.y] = new Vector2(g.x * cellSize, g.y * cellSize);
			}
		}
	}
	...
	public IEnumerable<Vector2> Samples() {
		// First sample only checks that the grid is not an illegal square
		Vector2 pos = Vector2.zero;
		bool validGridPosition = false;
		while (!validGridPosition) {
			pos = new Vector2(Random.value * rect.width, Random.value * rect.height);
			GridPos p = new GridPos(pos, cellSize);
			validGridPosition = grid[p.x, p.y] != Vector2.up;
		} 
		yield return AddSample(pos);
